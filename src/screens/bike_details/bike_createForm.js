...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, bikeId, numfields;
        const mapItems = MapItems;

        let bike = row['bike'];

        numfields = Helper.GetAllNumberFields(bike);
        if (numfields.length > 0) Helper.UpdateNumberFields(bike, numfields);


        // Add Or Update Bike
        rslt = await Support.AddOrUpdateBike(bike, []);
        if (rslt.status) {
            bikeId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Bike and navigation entity if it is deos not exist
            let navItem = bike.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "BikeID", value: parseInt(bikeId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateBike(data);
        ...
        global.AlertPopup("success", "Bike is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
