import BikeJsonConfig from "config/bike_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "bike", expand: "", exclude: [], func: Support.AddOrUpdateBike }
]

const FetchBikeInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(BikeJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of BikeJsonConfig[elm]) {
...
const FetchBikeDetails = async (bikeId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (bikeId) {
            ...
            if ($expand.length > 0) {
                rslt = await Api.GetBikeSingle(bikeId, `$expand=${$expand}`);
            } else {
                rslt = await Api.GetBikeSingle(bikeId);
            }
            
            if (rslt.status) {

                const bike = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? bike : bike[source];
...
const Extract = async (bikeId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchBikeInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(bikeId)) {
                    await FetchBikeDetails(bikeId, enums).then(({ row, backRow }) => {
