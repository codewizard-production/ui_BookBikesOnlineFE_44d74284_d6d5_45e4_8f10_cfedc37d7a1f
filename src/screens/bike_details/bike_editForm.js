...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], bikeId, bike, numfields, mapItem;

        bike = row['bike'];
        bikeId = row['bike'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) bikeId = rslt.id;
                            
                            const mapPropKey = bike.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
                                { key: "BikeID", value: parseInt(bikeId) },
                                { key: mapPropKey, value: parseInt(rslt.id) }
                            ];
                            rslt = await Support.AddOrUpdateBike(data, dropDownOptions);
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'bike');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
        ...
        global.AlertPopup("success", "Bike is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
